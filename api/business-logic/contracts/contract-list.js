...
async function queryAllContracts(network, basePath, {search, sort = 'created', order, cursor, limit}) {
    ...
    if (search) {
        ...
        // Search by protocol and home domain via directory
        ...
        if (idsToSearch.length > 0) {
            // Find contracts associated with these directory entries (via issuer or creator)
            const contracts = await db[network].collection('contracts')
                .find({
                    $or: [
                        {issuer: {$in: idsToSearch}},
                        {creator: {$in: idsToSearch}}
                    ]
                }, {projection: contractProjectedFields})
            ...
            return preparePagedData(basePath, {sort, order, cursor, limit}, formattedContracts)
        }
    }
