...
async function queryAllAssets(network, basePath, query = {}) {
    validateNetwork(network)
    
    const {search, sort, order, cursor, limit, includeUninitialized} = query
    const normalizedLimit = normalizeLimit(limit, 50)
    
    try {
        if (sort === 'created')
            return await queryAllAssetsByCreatedDate(network, basePath, cursor, normalizedLimit, order, includeUninitialized)

        const q = new QueryBuilder({payments: {$gt: 0}})
            .setSkip(calculateSequenceOffset(0, normalizedLimit, cursor, order))
            .setLimit(normalizedLimit, 50)
    ...
    return preparePagedData(basePath, {sort, order, cursor: q.skip, limit: q.limit}, assets)
        ...
        const horizonData = await horizonProxy.getAssetList(network, normalizedLimit, cursor)
        return horizonData
    }
}
