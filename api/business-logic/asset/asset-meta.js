...
async function queryAssetsMeta(network, basePath, query) {
    ...
    // Handle both ?asset[]=XLM and ?asset=XLM formats
    let assetParam = query.asset || query['asset[]']
    if (!assetParam) {
        throw errors.badRequest('Invalid parameter "asset". Expected an array of assets to fetch.')
    }
    
    // Normalize to array
    const assetArray = Array.isArray(assetParam) ? assetParam : [assetParam]
    
    if (!assetArray.length)
        throw errors.badRequest('No assets provided in the request.')
    if (assetArray.length > limit)
