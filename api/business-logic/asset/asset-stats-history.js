...
async function queryAssetStatsHistory(network, asset) {
    ...
    // Handle native XLM - return empty array as it doesn't have historical stats in the same format
    ...
    // Migration patch: Merges old asset_history format with new OHLCVT format
    // This ensures backward compatibility for clients still using the old format
    // The patch merges price data from asset_ohlcvt collection for timestamps after the switch
    // Keep this patch until all downstream clients have migrated to the new format
    const newFormatSwitchTimestamp = 1659916800 // June 6, 2022 - format migration date
