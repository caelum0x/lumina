...
class LedgerStream {
     ...
     */
    poll() {
        // Check if network database is available
        if (!db[this.network]) {
            // Only log once, not repeatedly
            if (!this._loggedUnavailable) {
                console.log(`Ledger stream: network ${this.network} not available (using Horizon instead)`)
        ...
        db[this.network].collection('ledgers')
            ...
            .then((result) => {
                if (!result || !result._id) {
                    // No ledgers in database yet, skip this poll
                    return
                }
                const ledger = result._id
                if (ledger > this.last) {
                    this.last = ledger
                    this.notify(ledger)
                }
            ...
            .catch((error) => {
                ...
                console.warn(`Ledger stream poll error for ${this.network}:`, error.message)
            })
    }
}
