...
// Get supported networks from config
...
const corsMiddleware = {
    whitelist: cors(function (req, callback) {
        ...
        if (!origin && req.billingProcessed)
...
module.exports = {
    ...
    registerRoute(app, route, options, handler) {
        ...
        // Validate network parameter early
        middleware.unshift((req, res, next) => {
            const networkName = req.params.network
            if (networkName && networks[networkName] === undefined) {
                return res.status(400).json({
                    error: `Unknown network name: "${networkName}".`,
                    status: 400
                })
            }
            next()
        ...
        app[method](prefix + route, middleware, function (req, res) {
            // Combine request path parameters with query params into a single params object
            const params = {
                ...req.params,
                ...req.query,
            ...
            let promise
            try {
                // Support both old format (req, res) and new format ({params, query, req, res})
                if (handler.length === 2) {
                    // Old format: handler(req, res)
                    promise = handler(req, res)
                } else {
                    // New format: handler({params, query, req, res})
