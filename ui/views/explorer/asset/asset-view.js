...
export default function AssetView() {
    ...
    // Fetch TOML metadata for asset issuer
    const {code, issuer} = asset?.descriptor || {}
    const tomlDomain = issuerInfo?.home_domain || pageMeta?.domain
    const issuerName = pageMeta?.name || (tomlDomain ? `Issuer (${tomlDomain})` : issuer)
    const title = !issuer ? 'XLM Stellar Lumens' : `${code} by ${issuerName}`
    usePageMetadata({
        title: 'Asset ' + title,
