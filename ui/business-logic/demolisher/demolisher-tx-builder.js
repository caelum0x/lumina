        ...
        for (let i = 0; i < opResult.length; i++) {
            ...
            if (resultCode !== 'op_success') {
                if (resultCode === 'op_sell_no_issuer') {
                    // Handle case where asset issuer no longer exists
                    // The operation tried to sell an asset whose issuer account was deleted
                    // We need to identify which asset/issuer caused this and add it to ignored list
                    const operation = operations[i]
                    if (operation && operation.selling) {
                        const assetIssuer = operation.selling.issuer || operation.selling.getIssuer()
                        if (assetIssuer && !this.ignoredIssuers.includes(assetIssuer)) {
                            this.ignoredIssuers.push(assetIssuer)
                            this.updateStatus(`Asset issuer ${assetIssuer.substring(0, 8)}... no longer exists, skipping`)
                        }
                    }
                }
            }
